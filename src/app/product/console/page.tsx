...
import { CampaignCurve } from "@/components/product/campaign-curve";
...
export default function ProductConsolePage() {
  ...
  const run = async () => {
    if (!selection || !data?.feasibility.ok || running) return;
    setRunning(true);
    setRunId((n) => n + 1);
    setPlayed(false);
    setOutcome(null);
  ...
  const onChartDone = useCallback(async () => {
    setPlayed(true);
    ...
    try {
      ...
      /* chart already finished */
    }
  }, []);

  const settle = async (o: "hit" | "miss") => {
    setOutcome(o);
    const c = campaignRef.current;
    if (!c) return;
    setBusy(true);
    try {
      const res = await fetch("/api/product/campaign/settle", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: c.id, outcome: o }),
      });
      const json = (await res.json()) as { ok: boolean; campaign?: Campaign };
      if (json.ok && json.campaign) setCampaign(json.campaign);
    } finally {
  ...
  const allIn = quote?.allInCents ?? campaign?.quote.allInCents ?? 0;
    ...
    <main className="mx-auto flex w-full max-w-[720px] flex-1 flex-col px-6 pb-16 pt-10 md:px-10">
      ...
      <div className="mt-8 rounded-card border border-ink/10 bg-surface shadow-raised">
        ...
        <div className="px-5 py-5 font-mono text-body leading-relaxed text-g700">
          ...
          <div
            className={`mt-4 flex flex-col gap-2 ${armed && !running ? "" : "pointer-events-none"}`}
          >
            {rows.map((row) => (
              <MarketRow
                key={row.ticker}
          ...
          {selection && data && (
            <>
              ...
              {running && played && !outcome && (
                <div className="mt-5 font-sans">
                  <PanelLabel>Settlement</PanelLabel>
                  <p className="mt-2 text-body text-g700">
                    All {ORDERS} orders are in — fully covered. The season ends
                    here.
                  </p>
              ...
              {outcome && (
                <div className="mt-5 font-sans">
                  ...
                  <p className="mt-3 text-d3 font-semibold tnum">
                    {fmtMoney(-allIn)}
                  </p>
