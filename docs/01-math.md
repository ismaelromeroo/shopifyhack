...
A hedged "free if X happens" promotion costs **exactly what an equal-EV flat discount costs, plus the fee stack.** Hedging removes variance; it never lowers the mean. A flat discount already has zero variance. So against a flat discount the hedged contingent promo is **dominated on both mean and variance**, and only a **behavioral edge** can rescue it: the contingent framing must sell *more units*.

The business is one number: **how much more.** For the running example that threshold is **+3.4% incremental units** to cover exchange friction alone, rising to **~10%** once a facilitator takes 2% of order value. §7 derives it; nothing else here matters as much.
...
## 2. The identity: contract price *is* probability
...
## 3. Promo unit economics and the break-even lift

**Expected cost per unit** = `p·R`. It depends on the *product*, so `(p=10%, R=$200)` and `(p=20%, R=$100)` cost the merchant identically. **Promo margin per unit** before fees is `m′ = m − p·R`; for a full-free promo that is `P·(g − p)`, which requires **`g > p`**.
...
| Structure | `R` | `p` | Cost/unit `p·R` | Margin `m′` | Break-even lift |
|---|---|---|---|---|---|
| Full-free, deep longshot | $200 | 3% | $6.00 | $84.00 | **+7.1%** |
| Full-free, longshot | $200 | 5% | $10.00 | $80.00 | +12.5% |
| **Full-free, standard (running example)** | **$200** | **10%** | **$20.00** | **$70.00** | **+28.6%** |
| Full-free, favorite | $200 | 20% | $40.00 | $50.00 | +80.0% |
| Full-free, near-coinflip | $200 | 35% | $70.00 | $20.00 | +350% |
| 50% rebate ($100 back) | $100 | 10% | $10.00 | $80.00 | +12.5% |
| 50% rebate | $100 | 20% | $20.00 | $70.00 | +28.6% |
| 25% rebate ($50 back) | $50 | 20% | $10.00 | $80.00 | +12.5% |
| 25% rebate | $50 | 35% | $17.50 | $72.50 | +24.1% |

**Read the iso-cost diagonals.** `(10%, full-free)`, `(20%, 50%-back)` and `(40%, 25%-back)` all cost $20/unit and all demand +28.6%. **The merchant is indifferent between them; the customer is not.** That asymmetry is the only design freedom in the model, and the evidence says which way to use it: a probabilistic promo beats a sure discount **only when the equivalent sure discount is, or seems, trivial** ([Gaertig & Simmons, *JCR* 52(5):1022–1042, 2026, N=8,969, preregistered](https://academic.oup.com/jcr/article/52/5/1022/8171334)). A 3% full-free offer has a $6 sure-equivalent on a $200 item — trivial. A 35% offer has a $70 sure-equivalent, and there the same paper finds the sure discount preferred. **Push `p` down and `R` up:** identical cost, better story, and the only regime the evidence supports.

## 5. Hedge mechanics and the payoff table

**Sizing.** A YES contract pays $1, so covering liability `L` takes **`N = L` contracts** — one per dollar of exposure — at cost `L·p`.
...
**Worked example.** `L = $20,000`, fair `p = $0.10`, 1¢ spread so you cross and fill at **$0.105**. `N = 20,000`. Fee = `roundup(0.07 × 20,000 × 0.105 × 0.895)` = `roundup(131.565)` = **$131.57**.
...
**Identical in both branches — that is the entire product.** A `{$0 or $20,000}` gamble becomes a certain $2,231.57, **11.158% of liability** against 10.000% at fair value. (Filling passively at $0.10 gives $2,000 + $126.00 = **$2,126.00 = 10.630%**, the thesis figure.) The 1¢ cross costs **11.58% over fair**; at `p` = 0.05 the same 1¢ cross costs **17.28% over fair**, because a fixed $0.005 half-spread is divided by a shrinking `p`.

**Drag, closed form** — load over fair value as a fraction of premium: `0.07·(1−p) + (s/2)/p`.

> **Two conventions, reconciled.** This closed form evaluates the fee coefficient at *fair* `p`; the worked example above charges the fee on the price actually paid, `p + s/2`. The closed form is therefore a slight under-estimate — **11.3% vs the exact 11.58%** at `p` = 0.10, **16.65% vs 17.28%** at `p` = 0.05. The table below and the quoted all-in figures in [`00-thesis.md`](00-thesis.md) and [`02-business.md`](02-business.md) all use the closed form; `δ(p)` in §1 and the `φ`/`e` table in §7 use the exact figure. The gap is a few tenths of a point and never changes a decision, but the two numbers are not typos of each other.

| `p` | Drag @ 1¢ spread | Drag @ 5¢ spread |
|---|---|---|
| 3% | **23.5%** | 90.1% |
| 5% | 16.65% | 56.65% |
| 10% | 11.3% | 31.3% |
...
**The drag is worst exactly where the marketing is best.** The product must therefore display **effective discount**, not contract probability.

## 6. The central point: hedging does not lower the mean

- **Unhedged** expected promo cost = `p·L`, with variance — outcomes are `{0, L}`.
- **Hedged** promo cost = `p·L + fees`, **certain**.

So `E[hedged] − E[unhedged] = fees > 0`. **Hedging buys variance elimination and pays in fees.** Against an *unhedged* contingent promo that is a good trade once `L` threatens solvency — the hedge's value scales with `L`, not with `p`. At $20,000 it is optional; at $2,000,000 it is not.

But the merchant's real alternative is not an unhedged promo. It is **a flat discount** — same mean cost `p·R` per unit, and **zero variance already**. Therefore:

> **A hedged contingent promo is EV-worse than an equal-cost flat discount by exactly the fee stack, and ties it on variance. It is dominated.**

This is the honest crux, and the sentence to put in front of anyone who thinks the hedge is the clever part. Every pitch in this category — ours included — is implicitly a claim that an *unmeasured behavioral edge* exceeds a *known, computable fee*. The canonical case study agrees: Mattress Mack won ~$75M on ~$10M of hedges and drove ~$75M in furniture sales, and by his own account **did not make a dime** ([Forbes, Nov 2022](https://www.forbes.com/sites/willyakowicz/2022/11/07/mattress-mack-won-his-75-million-world-series-bet-and-didnt-make-a-dime/)). The return was earned media, not margin — exactly what the identity predicts.

## 7. The required behavioral edge — the falsifiable threshold

Compare two promos of equal *pre-fee* cost `p·R` per unit. **Flat discount:** `d = p·R` off every unit, margin `m′ = m − p·R`, no fees. **Hedged contingent:** same expected cost plus fee stack `φ = R·δ(p) + f·P`, margin `m″ = m′ − φ`. With `Q_flat` and `Q_hedge` the units each produces, equal profit requires:

```
Q_hedge · m″     =  Q_flat · m′
Q_hedge / Q_flat =  m′ / m″  =  m′ / (m′ − φ)
```

$$\boxed{\;e \;=\; \frac{Q_{hedge}}{Q_{flat}} - 1 \;=\; \frac{\varphi}{\,m' - \varphi\,} \;=\; \frac{\varphi}{m''}\;}$$
...
1. **Exchange friction alone is a low bar** — 2–3% incremental units at longshot probabilities. If the contingent framing has *any* real pull, it clears this.
...
3. **The bar explodes as `p·R → m`.** At `p` = 35% the promo has eaten 78% of gross margin, so `m″` is tiny and any fixed fee is enormous relative to it. **Stay far from `p = g`.**
...
**No measured, third-party, holdout-controlled incrementality figure exists for this mechanic at an ordinary merchant.** Every published lift number is self-reported traffic or ad-level ROAS, never benchmarked against an equal-EV discount. That absence is itself the finding: `e` is not merely the key parameter, it is an **unmeasured** one, in a category ~40 years old.

## 8. Frictions, with real numbers

**Live sizing and price drift.** `L` grows as orders arrive. Hedging the *forecast* upfront kills drift risk but converts demand-forecast error directly into cost error — over-hedge and you unwind at the bid, under-hedge and you buy at a drifted price. Hedging **incrementally** is immune to demand error by construction and carries only drift risk. Threshold rebalancing (fire only when unhedged liability exceeds ~5% of total) matches per-order risk with **~76 trades instead of ~500** on a 500-order promo. *(Own Monte Carlo; drift volatility assumed and uncalibrated — direction sound, magnitudes not published here.)*

**Batch the clips.** Because the fee rounds up on the whole order, small clips bleed: at `p` = 0.10 the effective fee is **10.0% of premium on a 1-contract clip, 7.0% at 10, and 6.3% at 100+**. Never hedge per order.

**Liquidity is bimodal and top-of-book is unstable.** Season-championship books are genuinely deep — `KXSB-27-HOU` showed **2,028,833 contracts resting at the $0.05 ask** when we sampled it ([endpoint](https://api.elections.kalshi.com/trade-api/v2/markets/KXSB-27-HOU/orderbook?depth=100) — live, so it will not return that figure again), enough to absorb $250k instantly. Single-game books are not: on the same day two markets in the same NFL series differed **52×** in top-of-book size, and a $50,000 hedge on the thinner one slipped **+0.93¢** on a nominally 1¢ spread — roughly doubling the crossing term above. Across a random sample of 2,212 open markets the median spread was **5¢** and median top-of-book **200 contracts**; 44% had no two-sided quote at all. *(All liquidity figures in this section are our own book-walks against the public API on 15 Aug 2026 — a point-in-time snapshot, not a published dataset. That they are perishable is the finding.)* **Depth must be a live pre-trade gate, not a static allowlist.**
...
**Basis risk.** If the refund trigger *is* the contract's settlement condition, basis is zero. Any gap — a different settlement source, or an edge case (forfeit, suspension, cancellation) handled differently by the promo terms than by the contract's `rules_primary` — is uninsured tail. **Compound triggers are the sharp case:** hedging "A **and** B" with separate A and B contracts is not a hedge, it is an overpay plus correlation risk. Kalshi supports native multivariate contracts, but sampled ones showed **zero volume and no quotes** — expressible, not executable. Ship single-leg triggers only.
...
In the clean structure the **merchant funds and owns their own hedge**, so the hedge is not the facilitator's cost. Revenue is a take on notional or order value; cost is software, 50-state compliance, and amortized CAC.

**Name the denominator.** The running example produces three, and a take rate is meaningless without saying which: **order value $20,000** (100 × $200), **liability $20,000** (equal only because `R = P`), and **premium $2,000** (`L·p`). Confusing premium with notional turns a "10% take" into a 1% take.
...
## 10. Sensitivity — what actually moves the outcome

| Rank | Parameter | Plausible range | Effect | Why |
|---|---|---|---|---|
| 1 | **`e_actual`** — the real behavioral edge | −20% to +50%, **unmeasured** | Binary: business exists or it doesn't | The only term that can beat a flat discount; everything else is a cost |
| 2 | **`p / g`** — probability vs. margin | 0.07 to 0.78 | `e` moves 1.8% → 26.6% | `m″` is the denominator of `e`; as it shrinks, fixed fees explode |
| 3 | **`s`** — spread | 1¢ to 5¢ | Drag 11.3% → 31.3% of premium at `p` = 10% | Half-spread is fixed in dollars, so `(s/2)/p` blows up at low `p` |
...
**The ranking is the strategy.** Parameters 2–6 are engineering: choose low `p`, high `R`, deep books, a thin take, batched clips — and the cost side is solved to the cent, offline, against a public unauthenticated API, before anyone signs anything with anyone.

Parameter 1 is not engineering. It is an experiment nobody has run in forty years of this category existing: **one promo, one equal-cost flat-discount control, incremental *margin* measured against a holdout, net of the trailing 60 days.** If the contingent framing clears `e`, this is a business. If not, it is a more expensive discount with counterparty risk attached. That test costs a weekend; every other number here is either computable from a public API or explicitly flagged above as unverified.
